...
public class ExtendedFADCFitter implements IFADCFitter {
    ...
    public int pedistalMinBin = 1;
    public int pedistalMaxBin = 5;
    ...
    public int     thresholdCrossing;
    public int     pulsePeakValue; 
    public int     pulsePeakPosition;
    ...
    public void fit(int nsa, int nsb, int tet, int pedr, short[] pulse) {
            ...
            if(tcross>0) {
                ...
                this.pulseMinBin = Math.max(0,tcross-nsb);
                this.pulseMaxBin = Math.min(pulse.length-1,tcross+nsa);
                for (int bin=this.pulseMinBin; bin<=this.pulseMaxBin; bin++) { // sum should be up to tcross+nsa (without +1), this was added to match the old fit method
                    adc+=pulse[bin]-ped;
